import { formGrey3, primaryColor, white } from 'constants/styles';

export const wrapperWidth = '125px';
export const wrapperHeight = '45px';
export const wrapperBorderRadius = `10px`;
export const wrapperBorderWidth = '1px';
export const wrapperBorderColor = formGrey3;
export const wrapperReverseBorderColor = primaryColor;
export const wrapperHorizontalPadding = '15px';
export const wrapperBackground = white;

export const imgHeight = '5px';
export const wrapperImgTop = `calc((${wrapperHeight} - ${imgHeight}) / 2)`;
export const wrapperImgRight = '20px';
export const imgWidth = '10px';

export const ulWrapperTop = `calc(${wrapperHeight} + 5px)`;

export const itemDefaultColor = white;
export const itemActiveColor = formGrey3;
